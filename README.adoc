= Luix Content Analytic Engine with Apache Flink

[[overview]]
== Overview
Content analytic engine is to consume a stream of inputs and evaluate them against a set of rules. Any input that matches the rules is flagged as a output. The engine is implemented using Apache Flink and is deployed as a standalone application.

[[modules]]
== Modules
Fraud detection demo system consists of three main components:

- luix-cae-common: Common library for the CAE Flink Job
- luix-cae-flink-job: Content analytic engine application with Apache Flink
